...
isBloodOrange (Orange "Sanguinello" segments) = True
isBloodOrange (Orange "Moro" segments) = True
isBloodOrange (Orange "Tarocco" segments) = True
isBloodOrange (Orange variety segments) = False
isBloodOrange (Apple variety hasWorm) = False 

                     

-- 2.
bloodOrangeSegments :: [Fruit] -> Int
bloodOrangeSegments xs = sum [ segments | (Orange variety segments) <- xs , isBloodOrange (Orange variety segments) ]
...
-- 4.
p1 = ((Var "Q" :|: Var "P") :&: (Var "P" :&: Var "Q"))
p2 =  (( Var "P" :|: Var "Q") :&: (( Not(Var "P")) :&: (Not (Var "Q"))))
p3 = ( Var "P" :&: ( Var "Q" :|: Var "R")) :&: ( (( Not( Var "P")) :|: (Not( Var "Q"))) :&: (( Not( Var "P")) :|:  (Not ( Var "R"))))

-- satisfiable p1) True p2) False p3)False
-- tables for p1, p2 and p3 worked.


-- 5. 
tautology :: Prop -> Bool
tautology  xs = and [ eval x xs | x <- envs (names xs) ]

prop_taut1 :: Prop -> Bool
prop_taut1 x  = tautology x || satisfiable (Not x)

prop_taut2 :: Prop -> Bool
prop_taut2 x = satisfiable x || tautology (Not x) 
...
equivalent  xs ys = and[eval x xs | x <- envs (names xs)] == and[ eval y ys | y <- envs (names ys)]

--Doesn't do what it's suppose to do it

equivalent' :: Prop -> Prop -> Bool
equivalent' xs ys = undefined

prop_equivalent :: Prop -> Prop -> Bool
prop_equivalent  xs ys = equivalent xs ys == equivalent' xs ys 
