...
takeEmails xs  = [ x | x <- xs , contains x "mail"]


-- 8.
link2pair :: Link -> (Name, Email)
link2pair xs | (contains xs "mailto") = ((takeUntil "</a>" (dropUntil "\">" xs)) ,(takeUntil "\">" (dropUntil "mailto:" xs))) 
             | otherwise = error " Hi, this is not what I'm suppose to be given"
            


-- 9.
emailsFromHTML :: HTML -> [(Name,Email)]
emailsFromHTML xs = nub[link2pair x | x <- (takeEmails (linksFromHTML xs)) ]
...
findEmail y xs = [ (a,b) | (a,b) <- xs, contains a y  ]
